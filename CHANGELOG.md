...
- Исправлены кавычки разных уровней для поддерживаемых локалей и обработка глубокой вложенности в правиле `common/punctuation/quote`.
- Для французской локали узкие неразрывные пробелы теперь добавляются только внутри внешних кавычек `«…»`.
- Уточнены наборы символов `{locale}/char` для всех поддерживаемых локалей; исправлены данные для `ca`, `de`, `et`, `fi`, `fr`, `lv`, `nl`, `no`, `ro`, `sk`, `sv` и `tr`.
